...
def _tap(professional) -> str:
    """What the router reads for a prof-row tap (schemas.webhook.inbound_routing_text).
    ...
    """
    return inbound_routing_text(
        truncate_list_row_title(str(professional.name)), f"prof|{professional.id}"
    )
