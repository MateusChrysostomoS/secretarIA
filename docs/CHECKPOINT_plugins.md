...
| `precheck_handoff` | **core** — nenhum addon (`entitlement_keys=()`); o portão real (`ent.products.precheck`) mora na brain-api e volta como `NOT_ENTITLED` | post-booking: pede à brain-api que o PreCheck pré-semeie a sessão do paciente e manda UMA segunda mensagem de WhatsApp com o link `wa.me` da pré-consulta. Só fala em `SEEDED`/`ALREADY_ACTIVE` — qualquer outro desfecho é no-op silencioso. Idempotente por consulta (`ProcessedEvent` chave `precheck:{appointment_id}`), sem job de retry. Write-up: `docs/CHECKPOINT_precheck_handoff_post_booking.md`. |
